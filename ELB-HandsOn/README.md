...
Purpose of this hands-on is to learn Application Load Balancer (ALB) working process. Especially, we’ll cover the details of the AWS solution suite and walk through how to set up a basic ALB.
...
```text
Security Group Name  : ALBSecGroup #write a name for your group
Description         : ALB Security Group #write a description for your group
VPC                 : Default VPC
Inbound Rules: #The protocol and range of ports to allow. The traffic source to allow for inbound rules
    - Type: SSH----> Source: Anywhere ipv4
    - Type: HTTP ---> Source: Anywhere ipv4
Outbound Rules: # The traffic destination to allow for outbound rules
    Keep it as it is
Tag:
    - Key   : Name
      Value : ALB SEC Group
...
```text
Launch template name            : MyTemplate #write a name for your template
Template version description    : MyTemplate #write a description for your template
```
...
```text
t2.micro #because of free tier eligible
```

- Key Pair

```text
Please select your key pair (pem or cer key) that is created before
Example: mykey.pem #mine is PrivatePirate.cer
```
...
```text
Choose a target type    : Instances #selected Instances to register targets by instance ID
...
#I have 2 instances so ip's will always change respectively.
```
